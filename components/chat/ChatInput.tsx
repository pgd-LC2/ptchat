...
export default function ChatInput({ className, value, disabled, isWelcomeScreen = false, onChange, onSubmit }: Props) {
  ...
  return (
    <div className={`relative w-full mx-auto ${isWelcomeScreen ? 'max-w-[720px]' : 'max-w-[720px] pb-3 px-6'} ${className || ''}`}>
      ...
      <form
        className={`flex items-center gap-2 rounded-[10px] overflow-hidden border border-zinc-200 bg-white px-4 py-3 shadow-sm hover:border-zinc-300 focus-within:border-zinc-400 min-h-[52px] ${isWelcomeScreen ? 'mx-4' : ''}`}
        onSubmit={(e) => {
          e.preventDefault();
          onSubmit(e);
        }}
